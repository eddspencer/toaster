...
export default {
  createPoint: function(x: number, y: number): Point {
    return new Point2d(x, y)
  },
  createVector: function(start: Point, end: Point): Vector {
    return new Vector(start, end)
  },
  createVectorFromPoint: function(start: Point, theta: number, length: number): Vector {
  ...
  createRectangle: function(x: number, width: number, y: number, length: number): Rectangle {
    return new Rectangle(x, width, y, length)
  },
  distanceBetweenPoints: function(p1: Point, p2): number {
    const xDist: number = p2.x - p1.x
    const yDist: number = p2.y - p1.y
    return Math.sqrt(xDist * xDist + yDist * yDist)
  },
  norm: function(p: Point): number {
    return Math.sqrt(p.x * p.x + p.y * p.y)
  },
  boundAngle: function(theta: number): number {
   ...
   */
  getIntersectPoint: function(line1: Vector, line2: Vector): Point {
  ...
  addVectors: function(v1, v2): Vector {
   ...
   */
  transform: function(p: Point, theta: number): Point {
